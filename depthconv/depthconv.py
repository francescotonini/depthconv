...
class DepthConv(Module):
    ...
    def forward(self, input, depth):
        ...
        f = DepthconvFunction(_pair(self.stride), _pair(self.padding), _pair(self.dilation))
        
        # print bias
        if isinstance(self.bias, torch.nn.Parameter):
            return f(input, depth, self.weight, self.bias)
        else:
            return f(input, depth, self.weight)
